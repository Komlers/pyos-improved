...
import re
import core.autoexec.autoexec as autoexec
try:
    from core.module.actions import *
    from core.module.style import *
    from core.module.textmoji import *
    from core.module.splashes import *
except Exception:
    print("Kernel Panic!!")
    sys.exit(15)
...
import art
import pprint
import core.shizuku.manager as szkmng # Installer for shizuku
...
logging.basicConfig(filename='./data/cache/.output.log', datefmt='%b %a %d %H:%M:%S %Y', level=logging.INFO, format=LOG_FORMAT)
...
co_manualHelp = "core/plaintext/manualhelp.txt"
co_welcome = "core/plaintext/welcome.txt"
# core/plaintext loads END

print("PT Registered")

def cmdhistory_write():
    tmp_f = open("./data/cache/history.txt", "a", encoding="utf-8")
    # cmdhist_lines += 1
        ...
        while count < 3: # 代码难以维护，到处不明变量 --wusheng233
            ...
            try:
                ...
                if shorter_welcome == "false":
                    cat(co_welcome) # Welcome text, editable at core/plaintext
                elif shorter_welcome == "true":
                    cat("core/plaintext/welcome_shorter.txt")
                ...
                try:
                    cat("core/plaintext/lastlogin.txt")
                except FileNotFoundError:
                ...
                tmp_outolog = open("./data/cache/.output.log", "a", encoding="utf-8")
                with open("core/plaintext/lastlogin.txt", "w", encoding="utf-8") as ll_wrt:
                ...
                while count < 3:
                  ...
                  # lsh_username = os.system("whoami")
                    ...
                    if cmd == "ls": # Path
                        ...
                        cat("./core/plaintext/netrefresh_help.txt")
                    ...
                    # Package manager info
                        ...
                        cat("./core/plaintext/extprog_info.txt")
                    ...
                    # The credits
                        ...
                        cat("./core/plaintext/shizuku_credits.txt")
                    # Package search
                        ...
                        if user == "dev":
                            ...
                            os.chdir("./apps")
                            cat_bugged("core/plaintext/manualhelp.txt")
                        ...
                        cat("core/plaintext/before_talking.txt")
                ...
                runPreInstApp("core/catchinfo.py")
