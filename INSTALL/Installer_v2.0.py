...
while True:
    ...
    if str(lang) == "1":
        ...
        print("欢迎使用 PY OS Improved 安装程序！")
        break
    elif str(lang) == "3":
        print("歡迎使用 PY OS Improved 安裝程式！")
        break
    elif str(lang) == "4":
        print("ようこそ PY OS Improved インストーラへ！")
        break
    elif str(lang) == "5":
...
if lang == "1":
    ...
    step0_1 = "Do You Need to Set the Image Source of PIP to the Tsinghua Source?\n(We strongly recommend Chinese mainland users to change the source for a faster and more stable download environment)\n[0] Skip (provided you know the source you are using)\n[1] Change the image source to Tsinghua University mirror site\n[2] Use/replace the pypi default image source (do not change the source)"
    ...
    step0_1 = "您是否需要將pip的鏡像源設置為清華源？\n(我們強烈建議中國大陸使用者換源以獲得更高速更穩定的下載環境)\n[0]跳過(前提是您清楚您正在使用的鏡像源)\n[1]更換鏡像源為清華大學鏡像站\n[2]使用/更換pypi默認鏡像源(不換源)"
    step0_2 = "正在更新pip"
    step0_3 = "當前pip版本："
    step0_4 = "正在切換pip镜像源..."
    ...
    step0_1 = "pipのミラーソースを清華源に設定する必要がありますか？\n(私たちは、中国本土のユーザーにより高速で安定したダウンロード環境を得るために、ソースを変更することを強く推奨します)\n[0]スキップする(前提として、使用しているミラーソースが分かっていること)\n[1]ミラーソースを清華大学のミラーサイトに変更する\n[2]pypiのデフォルトミラーソースを使用する/変更する(ソースを変更しない)"
    step0_2 = "pip の更新"
    ...
    step0_1 = "Вам нужно установить источник изображения PIP на источник Tsinghua?\n(Мы настоятельно рекомендуем пользователям из материкового Китая сменить источник для более быстрой и стабильной загрузки данных)\n[0] Пропустить (при условии, что вы знаете источник, который используете)\n[1] Измените источник изображения на зеркальный сайт Университета Цинхуа\n[2] Используйте/замените источник изображения pypi по умолчанию (не меняйте источник)"
...
def step3():
    ...
    if platform.system() == "Windows":
        ...
        choose02 = input("> ")
        if choose02 == "":
            with open('..\\PYOSI\\config\\config.json', 'r') as file:
                config = json.load(file)
            # 当前系统为Windows，修改配置为true
            config['isWindows'] = "true"
            with open('..\\PYOSI\\config\\config.json', 'w') as file:
